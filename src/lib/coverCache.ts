...
function findCoverUrl(input: unknown, depth = 0): string | null {
...
async function downloadCover(coverUrl: string, targetDir: string, bookId: string): Promise<string | null> {
...
export function extractBookCoverUrl(bookInfo: unknown): string | null {
  return findCoverUrl(bookInfo);
}

export async function cacheBookCover(bookId: string, bookInfo: unknown): Promise<CachedCover> {
